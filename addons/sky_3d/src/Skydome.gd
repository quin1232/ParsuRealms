...
func set_moon_altitude(value: float) -> void:
	# Clamp the incoming value to the desired -47 to +47 range
	var clamped_value: float = clampf(value, -47.0, 47.0)
	
	# Only update if the clamped value is actually different from the current moon_altitude
	if clamped_value == moon_altitude:
		return
		
	moon_altitude = clamped_value # Store the clamped value
	update_moon_coords()          # Update the moon's position and other visuals
...
func atm_moon_phases_mult() -> float:
	...
	# If phases are enabled:
	var calculated_phase_brightness = TOD_Math.saturate(-sun_direction().dot(moon_direction()) + 0.60)
	
	var min_brightness_due_to_phase = 0.2 # ADJUSTABLE: Minimum brightness factor from phases (e.g., 0.2 means even a new moon has 20% of full moon's potential phase brightness).
	return max(calculated_phase_brightness, min_brightness_due_to_phase)
...
func update_moon_light_energy() -> void:
	...
	var final_calculated_energy: float

	var lower_altitude_limit_deg: float = -47.0
	var upper_altitude_limit_deg: float = 47.0
	var effective_altitude_norm: float
	if upper_altitude_limit_deg == lower_altitude_limit_deg:
		effective_altitude_norm = 0.5 
	else:
		effective_altitude_norm = clampf( (moon_altitude - lower_altitude_limit_deg) / (upper_altitude_limit_deg - lower_altitude_limit_deg), 0.0, 1.0)

	var brightness_scale_at_lower_limit: float = 0.2  
	var brightness_scale_at_upper_limit: float = 1.0  
	var altitude_contribution_factor = lerpf(brightness_scale_at_lower_limit, brightness_scale_at_upper_limit, effective_altitude_norm)
	var light_after_altitude_mod = moon_light_energy * altitude_contribution_factor 

	var phase_multiplier = atm_moon_phases_mult() 
	var light_after_phase_mod = light_after_altitude_mod * phase_multiplier
	
	var fade_curve_input: float = (1.0 - sun_direction().y) * 0.5 
	final_calculated_energy = light_after_phase_mod * Sky3D._sun_moon_curve_fade.sample_baked(fade_curve_input)

	var absolute_minimum_overall_moon_light = 0.25 # Your desired minimum, applies day and night.

	__moon_light_node.light_energy = max(final_calculated_energy, absolute_minimum_overall_moon_light)
