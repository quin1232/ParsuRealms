...
func _on_body_exited(body: Node) -> void:
	...
	if _btn:
		_btn.visible = false
		_btn.disabled = true
		Dialogic.VAR.ITquest = false
		Dialogic.VAR.Isfoundtoolbox = false
	if trigger_once:
		_fired = true
		if _btn:
			_btn.visible = false
