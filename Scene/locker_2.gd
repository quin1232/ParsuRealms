...
func _on_body_entered(body: Node) -> void:
	if Dialogic.VAR.Quest1complete == true:
		return
	if _fired:
		return
	...
	Dialogic.VAR.Isfoundlocker = true
	Dialogic.start("CEC", "locker")
	allowed_body.set_physics_process(false)
	$"../PlayerTemplate".play_idle(true)

	# Show the button when the player is inside
...
func _on_body_exited(body: Node) -> void:
	...
	if _btn:
		_btn.visible = false
		_btn.disabled = true
	Dialogic.VAR.Isfoundlocker = false


	# If one-shot, hide the button and disable further monitoring
	if trigger_once:
		_fired = true
		if _btn:
			_btn.visible = false
